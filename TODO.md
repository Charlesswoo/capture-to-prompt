...
#### 진짜 원인은 추출 과장이었다 (2026-09-17, 원본·생성본 대조)

사용자 지적("추출 당시부터 잘못 추출된 것 아니냐")이 맞았다. 원본과 생성본을
직접 열어 비교했다:

- 원본(11CC4FD8): 선화가 뚜렷한 애니 일러스트, 니트는 단순 리브, 피부는 평면 채색
- 생성본: 선화가 사라지고 니트가 **실제 털실 섬유**로, 피부가 실사급 질감으로

결정적인 건 **추출 안에서 어긋났다**는 것 — `medium`은
...
- **질감은 원본이 보여주는 수준까지만.** 니트가 단순 리브면 "simple ribbed knit"이라
  적고 개별 섬유를 쓰지 않는다. 없는 재질 디테일을 지어내는 것이 결과를 사진 쪽으로
  민다
...
### 가려져 있던 진짜 원인 (2026-09-17)

노이즈를 걷어내니 나왔다: **ChatGPT 구독 사용량 한도 초과**
(`You've hit your usage limit … try again at Sep 19th, 2026 10:27 PM`).

- [x] 중복 줄 제거 — codex가 같은 오류를 두 번 뱉어 메시지가 배로 길었다
- [x] `usageLimitHint(_:)` — 한도 초과는 기다리거나 결제하는 수밖에 없지만,
      **다른 백엔드로 계속 쓸 수 있다는 건 앱만 안다.** 그것만 알려준다
- [x] 테스트 242개 통과
