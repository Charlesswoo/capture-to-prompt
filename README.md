...
빈 화면(⌘N)에서 만들고 싶은 장면을 문장으로 쓰면 그것만으로 이미지를 만든다.
만들어진 이미지는 그 항목의 **원본**이 되고, 곧바로 분석해 3개 언어 프롬프트를 채운다.
입력한 문장은 씨앗일 뿐이고 정확한 프롬프트는 결과 그림에서 다시 뽑는다 — 이후
비교·변형·재추출은 캡처와 똑같이 동작한다. 씨앗 문장은 프롬프트 로그에
`note=source=prompt_only`로 남는다.
